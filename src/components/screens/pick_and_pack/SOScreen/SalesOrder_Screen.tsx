...
const SalesOrdersScreen: React.FC = () => {
  ...
  const [issueCompletedMap, setIssueCompletedMap] = useState<
    Record<string, boolean>
  >({});
  const [packCompletedMap, setPackCompletedMap] = useState<
    Record<string, boolean>
  >({});
  ...
  const closeModal = () =>
    setModal((m) => ({ ...m, visible: false, onConfirmDelete: undefined }));

  const computeIssueCompletion = (
    ...
    try {
      ...
      await Promise.all(
        orders.map(async (o) => {
          ...
          if (hasDetails) {
            const stored = await getOrderDetails(so);
            const issueComplete = computeIssueCompletion(
              stored?.orderDetails
            );
    ...
    async (o: OrdersSummaryItem) => {
      ...
      try {
        ...
        if (!stored?.orderDetails?.length) {
          throw new Error(
            "No order details found. Please download first."
          );
        }
        ...
        setList((prev) =>
          prev.filter((it) => it.saleOrderNumber !== so)
        );
        setPhaseMap((prev) => {
          const n = { ...prev };
          delete n[so];
        ...
        // Refresh maps for remaining rows
        const remaining = list.filter(
          (it) => it.saleOrderNumber !== so
        );
