...
import type { Order } from "../../Api/SalesOrder_server";
...
const SalesOrdersStyledTable: React.FC<Props> = ({ data = [], onDownload }) => {
  ...
  const renderTableHeader = () => (
    <View style={styles.tableHeader}>
      <HCell flex={2}>Sales Order Number</HCell>
      <HCell flex={1} center>Priority</HCell>
      <HCell flex={1} center>Total Items</HCell>
      <HCell flex={1} center>Total Materials</HCell>
      <HCell flex={2} center>Status</HCell>
      <HCell flex={2} right>Actions</HCell>
    </View>
  );
  ...
  const renderRow = ({ item, index }: { item: Order; index: number }) => {
    ...
    return (
      ...
      >
        ...
        <RCell flex={1} center>
          {item.priority ? <PriorityCircle level={item.priority} /> : <Text style={styles.cellText}>—</Text>}
        </RCell>
        ...
        <RCell flex={1} center>
          <Text style={styles.cellText}>{item.totalMaterials}</Text>
        </RCell>
        ...
        <RCell flex={2} right>
          ...
          >
            ...
            <Text style={styles.downloadTx}>Download</Text>
          </Pressable>
        </RCell>
      </View>
    );
      ...
      <View style={styles.tableWrap}>
        ...
        {(!data || data.length === 0) ? (
          <View style={{ padding: 16 }}>
            <Text style={{ color: C.subText, fontSize: 14 }}>No orders found.</Text>
          </View>
        ) : (
          <FlatList
            data={data}
...
const styles = StyleSheet.create({
  ...
  hcell: { paddingHorizontal: 8 },
  htext: { fontSize: 12, color: C.headerText, fontWeight: "700" },
  ...
  sep: { height: 1, backgroundColor: C.border, marginLeft: 12, marginRight: 12 },
  ...
  statusTx: { color: C.chipTx, fontSize: 12.5, fontWeight: "700" },
